...
These scripts follow the [community-scripts/ProxmoxVE](https://github.com/community-scripts/ProxmoxVE) convention to deploy **Music Downloader** as a bare-Python service inside a Debian 13 LXC container on Proxmox VE 9+.
...
The interactive wizard will ask about container resources (or accept defaults).
...
The `BRANCH` variable controls which branch of the repository is cloned inside the container. It defaults to `main`. To deploy a different branch (e.g. for testing):
...
1. **`ct/music-downloader.sh`** (runs on Proxmox host):
   ...
   - Downloads a Debian 13 template if needed
   - Creates and starts the LXC container
   - Handles SSH key deployment and console autologin via `build.func`
   - Fetches and runs the install script inside the container via `lxc-attach`

2. **`install/music-downloader-install.sh`** (runs inside the container):
   - Updates the OS
   - Installs Python 3, git, pip
   - Clones the repository (selected branch) to `/opt/music-downloader`
   - Creates a Python venv and installs `requirements.txt`
   - Creates and enables a `systemd` service (`music-downloader`)
   - Configures MOTD and console autologin via community-scripts hooks
...
The LXC IP is shown at the end of the install output.

## Updating

Re-run the container script from the Proxmox host and select **Update** when prompted:
...
Check which branch and commit is installed:

```bash
cat /opt/music-downloader_version.txt
```
